...
void Draw_smooth::addVBO(unsigned int currentObject)
{
    ...
    else
    {
        ...
        for (auto&& f: obj.faces()) 
        {
            ...
            const Vector N0 = Ns[f.normalIndex(0)];
            const Vector N1 = Ns[f.normalIndex(1)];
            const Vector N2 = Ns[f.normalIndex(2)];
            
            Vector faceNormal = (N0 +N1+N2)/3.0;
            faceNormal= faceNormal.normalized();
            
            
            auto pushVertex = [&](int vertexIdx, int tcIdx, const Vector& faceNorm)
            {
                const Vertex v = verts[vertexIdx];
                const pair<float, float> TC = ( tcIdx <texcords.size() ) ? texcords[tcIdx] : make_pair(0.f,0.f);
                ...
                normals.push_back(faceNorm.x());
                normals.push_back(faceNorm.y());
                normals.push_back(faceNorm.z());

                colors.push_back(fabs(faceNorm.x()));
                colors.push_back(fabs(faceNorm.y()));
                colors.push_back(fabs(faceNorm.z()));

                texCoords.push_back(TC.first);
                texCoords.push_back(TC.second);
            };

            pushVertex(f.vertexIndex(0), f.texcoordsIndex(0), faceNormal);
            pushVertex(f.vertexIndex(1), f.texcoordsIndex(1), faceNormal);
            pushVertex(f.vertexIndex(2), f.texcoordsIndex(2), faceNormal);
        }
    }
